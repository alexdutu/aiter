    ...
    const float* __restrict__ fp8_out_scale_ptr)
{
    ...
    // for QK mfma16x16, layout is QHead/Tokenx16 across every 16 lanes, 16 Bytes HeadElements in
    // each lane, 4x16B HeadElements across 4 rows of warp
    ...
    _B16x8 Qlocal[QKHELOOP]
                 [QK_SIZE_RATIO]; // note that 16 contiguous elements of Q should be fetched per
                                  // lane for 8 bit cache types : QK_SIZE_RATIO changes for this
    ...
    // output layout from QKmfma : QH16xT4x4 16 qheads across 16 lanes, 16 tokens across 4 rows x 4
    // tokens per lane
    ...
    constexpr int VTLANELOOP = DIVIDE_ROUND_UP(
        VTOKENS_PER_LANE, CONTIGUOUS_KV_ELEMS_16B_LOAD); // optimized for 16B fetches; assumes
                                                         // minimum block size is 16
    constexpr int VHELOOP =
        HEAD_SIZE / 16 /
        NWARPS; // head_size distributed across warps; each mfma instr works on 16 head elements
            ...
            if constexpr(KV_DTYPE == vllm::Fp8KVCacheDataType::kAuto)
            {
                for(int vfetch_depth = 0; vfetch_depth < VTLANELOOP; vfetch_depth++)
                {
                    for(int i = 0; i < ELEMS8_ELEMS4_RATIO; i++)
                    {
                        ...
                        // output format is 16 qheads across 16 lanes, 16 head elems spread across 4
                        // rows
