    ...
    const FlashMlaPrefillFwdParams& params)
{
    ...
    const int32_t num_blk  = ck_tile::integer_divide_ceil(params.size_s,  Traits::kBlockM) *
                             ...
                             params.num_splits;
    const dim3 grid_core = dim3(num_blk, params.size_h, params.size_b);
    const dim3 grid_comb = dim3(params.size_s, params.size_h, params.size_b);

    if (params.num_splits > 1)
    {
        auto kn_s1 = &kn_fmla_fwd_splictkv_prefill<Traits, scalar_t, acc_t, kIsCausal, true>;
        auto kn_s2 = (params.num_splits <= 32)  ? &kn_fmla_fwd_splictkv_prefill_combine<Traits, 32,  scalar_t, acc_t> :
        ...
        TORCH_CHECK(kn_s2 != nullptr, "num_splits is larger than expected (<=128) !");
        kn_s1<<<grid_core, Traits::kNumThreads, 0, stream>>>(params);
        kn_s2<<<grid_comb, Traits::kNumThreadsCombine, 0, stream>>>(params);
    }
    else
    {
        auto kn_s1 = &kn_fmla_fwd_splictkv_prefill<Traits, scalar_t, acc_t, kIsCausal, false>;
        kn_s1<<<grid_core, Traits::kNumThreads, 0, stream>>>(params);
    }
}
