...
def input_helper(BATCH, SEQLEN, H, HEAD_DIM, dtype, absorb=False):
    ...
    b_start_loc = torch.cat([torch.tensor([0], dtype=torch.int32), b_seq_len.cumsum(dim=0, dtype=torch.int32)])
    b_start_loc = b_start_loc.to(device="cuda")
    b_seq_len = b_seq_len.to(device="cuda")

    return q, k, v, b_seq_len, b_start_loc[:BATCH]

def varlen_input_helper(BATCH, SEQLEN, H, HEAD_DIM, dtype, absorb=False, equal_seqlens=False):
...
@pytest.mark.parametrize('Z, H, SEQLEN, HEAD_DIM', [
    (4, 48, 1024, 64),
    (1, 24, 8192, 64),
    (1, 4, 16384, 128),
    (2, 16, 1020, 128),
...
def test_op_fwd(Z, H, SEQLEN, HEAD_DIM, causal, absorb, varlen, dtype=torch.float16):
    ...
    if varlen:
        q, k, v, b_seq_len, b_start_loc = varlen_input_helper(Z, SEQLEN, H, HEAD_DIM, dtype, absorb)
    else:
        q, k, v, b_seq_len, b_start_loc = input_helper(Z, SEQLEN, H, HEAD_DIM, dtype, absorb)
    ...
    for i in range(0, Z):
        ...
        if causal:
            ...
            causal_mask = torch.triu(torch.ones(seqlen_q, seqlen_k, device=scores.device), diagonal=1).bool()
