...
def input_helper(B, H_Q, H_KV, D, KV_BLK_SZ, SEQ_LEN, dtype, kv_cache_dtype, output_type, num_blocks=4):
    ...
    block_tables = []
    for i in range(B):
        block_table = random.sample(range(0, num_blocks), num_blocks) + [random.randint(0, num_blocks - 1) for _ in range(max_num_blks_per_seq - num_blocks)]
...
def input_la_helper(B, H_Q, H_KV, D, KV_BLK_SZ, SEQ_LEN, dtype, kv_cache_dtype, output_type, num_blocks=4):
    total_programs=304
    BLOCK_M = 16
    BLOCK_N = KV_BLK_SZ
...
#    print(f'N_CTX: {n_ctx}')
    ...
    # Allocate Tensors
    q = torch.empty((H_Q, N_CTX_Q * B, D), dtype=dtype, device="cuda").normal_(mean=0.0, std=0.5)
    k = torch.empty((H_Q, sum_n_ctx, D), dtype=dtype, device="cuda").normal_(mean=0.0, std=0.5)
    v = torch.empty((H_Q, sum_n_ctx, D), dtype=dtype, device="cuda").normal_(mean=0.0, std=0.5)
    ...
    return q, k, v, kv_block_tables, Mp, Lp, Op, total_programs, locks, batch_num_block_n, BLOCK_M, BLOCK_N
...
def paged_attn_decode(OP, BS, H_Q, H_KV, D, KV_BLK_SZ, SEQ_LEN, num_blocks, dtype, kv_cache_dtype, compute_type, output_type):
    ...
    q, k, v, kv_block_tables, Mp, Lp, Op, total_programs, locks, batch_num_block_n, BLOCK_M, BLOCK_N = input_la_helper(BS, H_Q, H_KV, D, KV_BLK_SZ, SEQ_LEN, dtype, kv_cache_dtype, output_type, num_blocks)
...
#                ('yellow', '-')], ylabel='ms / TFLOPS / GB/s', plot_name=f'{model_name}-benchmark', args={})
        styles=[('red', '-')], ylabel='ms', plot_name=f'{model_name}-benchmark', args={})
    ...
    def bench_paged_attn_decode(OP, BS, HQ, HK, SEQ_LEN, HEAD_DIM, metric):
        ...
        KV_BLK_SZ = 16
        num_blocks = 64
        fn = paged_attn_decode(OP, BS, HQ, HK, HEAD_DIM, KV_BLK_SZ, SEQ_LEN, num_blocks, dtype, kv_cache_dtype, compute_type, output_type)
