...
def extend_forward(q_extend, k_extend, v_extend, k_buffer, v_buffer, kv_indptr, kv_indices, qo_indptr, custom_mask, mask_indptr, max_len_extend, causal, sm_scale=1.0, logit_cap=0.0):    
    out = torch.empty((*q_extend.shape[:-1], v_extend.shape[-1]), dtype=q_extend.dtype, device=q_extend.device)
    extend_attention.extend_attention_fwd(q_extend, k_extend, v_extend, out, k_buffer, v_buffer, qo_indptr, kv_indptr, kv_indices, custom_mask, causal, mask_indptr, max_len_extend, sm_scale=sm_scale, logit_cap=logit_cap)
    return out

def prefill_forward(q_extend, k_extend, v_extend, b_start_loc, b_seq_len, max_len_extend, causal):    
    out = torch.empty((*q_extend.shape[:-1], v_extend.shape[-1]), dtype=q_extend.dtype, device=q_extend.device)
    prefill_attention.context_attention_fwd(q_extend, k_extend, v_extend, out, b_start_loc, b_seq_len, max_len_extend, causal=causal)
    return out
...
def benchmark(args):
    ...
    plot_name = args.plot_name + f"-causal-{args.causal}-equal_seqlens-{args.equal_seqlens}"
        ...
        if provider == "extend_attention_fwd":
          def extend_attention():
              return extend_forward(
                  ...
                  kv_indices, qo_indptr, custom_mask, mask_indptr, max_len_extend, args.causal,
                  sm_scale, logit_cap
              )
          
          def context_attention():
              return extend_forward(
                  q_extend, k_extend, v_extend, B_Start_Loc, B_Seqlen, max_len_extend, args.causal
              )
          
          if provider == "extend_attention_fwd":
              fn = extend_attention
          elif provider == "context_attention_fwd":
              assert prefix == 0, "Prefix length must be 0 for context attention. Try setting -mode prefill."
...
def parse_args():
    ...
    parser.add_argument("-mode", type=str, default="extend", help="Mode of the benchmark. Options: extend, prefill")
